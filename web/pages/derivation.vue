...
import { recentSolstice, octaeteride, year, currentStart, dayCount, octaveCount, seasonCount, transits, jd0Astro, yearLength, startMoon, nextMoon, moonCycle, startSolstice, CALENDAR_EPOCH, yearsPassed, historicalTransits } from '../../src/astro.js';
...
section.border-1.bg-orange-50.shadow-xl.flex-auto.p-4.gap-2.flex.flex-col.overflow-y-scroll.text-sm.max-w-55ch.mx-auto.my-2px
  ...
  .text-lg So the time now is in the Second Era of Octaeteris <code class="octal">{{octaeteride.toString(8)}}-{{(dayCount+dayFraction).toString(8).slice(0,9)}}</code> as pure day count or <b>{{octaeteride.toString(8)}}-{{seasonCount.toString(8)}}-{{(dayCount-seasonCount*73+dayFraction).toString(8).slice(0,7)}}</b> with pentad seasons in octal.
  ...
  p So, in octal, we can say that this is Era <code class="octal">{{octal(1).padStart(2,'0')}}</code> Octaeteris <code class="octal">{{octal(octaeteride).padStart(2,'0')}}</code>, pentad <code class="octal">{{octal(seasonCount).padStart(2,'0')}}</code>, day <code class="octal">{{octal(dayCount-seasonCount*73).padStart(2,'0')}}</code>. It's day <code class="octal">{{dayCount%8}}</code> of the octave <code class="octal">{{octal(Math.floor(dayCount/8))}}</code>.
